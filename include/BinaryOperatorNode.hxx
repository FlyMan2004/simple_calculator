...
concept cxx_addable = requires(T_lhs lhs, T_rhs rhs) { lhs + rhs; };
  ...
  struct evaluate_impl
  {
    ...
    struct Plus
    {
      ...
      static auto operator()(T_lhs const& lhs, T_rhs const& rhs) noexcept(noexcept(lhs + rhs)) -> EvalResult
        requires cxx_addable<T_lhs, T_rhs>
      {
        return lhs + rhs;
      }
      ...
      static auto operator()(T_lhs const& lhs, T_rhs const& rhs) -> EvalResult
        requires (!cxx_addable<T_lhs, T_rhs> && (std::same_as<T_lhs, std::string> || std::same_as<T_rhs, std::string>))
      {
...
public:
  ...
  BinaryOperatorNode& operator=(BinaryOperatorNode&&) = default;
  BinaryOperatorNode(
    simple_calc::Op op,
